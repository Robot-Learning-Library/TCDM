...
def _obj_mimic_task_factory(domain_name, name, object_class, target_path):
    def task(append_time=True, pregrasp='initialized', ref_only=False, auto_ref=False, traj_path=None, reward_kwargs={}, environment_kwargs={}):
        ...
        # i = 0
        # p = [0,0,0]
        # mocap_name = '0_mocap'.format(i)
        # mocap_body = env.mjcf_model.worldbody.add('body', mocap=True, name=mocap_name, pos=p)
        # rgba = [0,0,1,1]
        # mocap_body.add('site', name='0', size=(0.005,), rgba=rgba, pos=[0,0,0])

        # # create equality constraint
        # parent_body = 'hammer/object'
        # env.mjcf_model.equality.add('connect', body1=mocap_name, body2=parent_body, anchor=[0,0,0], solimp=[0.9,0.95,0.001], solref=[0.02,1])
...
TCDM_DOMAINS_OBJ_ONLY = {}
TCDM_DOMAINS_OBJ_ONLY['door'] = containers.TaggedTasks()
...
for target_fname in _TCDM_TRAJS:
    ...
    if domain_name not in TCDM_DOMAINS_OBJ_ONLY:
        TCDM_DOMAINS_OBJ_ONLY[domain_name] = containers.TaggedTasks()
    try:
        object_class = mj_models.get_object(domain_name)
    except:
        object_class = mj_models.get_object(task_name)

    task = _obj_mimic_task_factory(domain_name, task_name, object_class, target_fname)
    TCDM_DOMAINS_OBJ_ONLY[domain_name].add('mimic')(task)

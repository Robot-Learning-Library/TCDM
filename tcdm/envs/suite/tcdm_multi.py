...
def _multi_obj_mimic_task_factory(env_name, object1_class, object2_class, robot_class):
    def task(append_time=True, pregrasp='initialized', ref_only=False, auto_ref=False, traj_path=None, reward_kwargs={}, environment_kwargs={}):
        ...
        fixed_object_model = object2_class()
        env.attach(fixed_object_model)

        if traj_path is None:
            data_path = traj_abspath(env_name+'.npz', traj_path='trajectories/multi_trajs')
        else:
            data_path = generated_traj_abspath(env_name, traj_path, env_name)

        if robot_class:
            task_class = MultiObjMimicTask
        else:
            task_class = MultiObjOnlyMimicTask
        task = task_class(object_name, data_path, reward_kwargs, append_time, pregrasp, ref_only, auto_ref, env_name, traj_path)
        ...
        return Environment(physics, task,
                           ...
                           **environment_kwargs)
    task.__name__ = env_name.split('_')[-1]
    return task
...
def get_domain_multi(env_name, obj_only=False):
    ...
    # target = traj_name.split('/')[-1][:-4]
    obj1_name, obj2_name = env_name.split('-')[0:2]
    # task_name = env_name.split('_')[-1]

    object1_class = mj_models.get_object(obj1_name)
    object2_class = mj_models.get_object(obj2_name)
    if obj_only:
        robot_class = None
    else:
        robot_class = mj_models.Adroit

    task = _multi_obj_mimic_task_factory(env_name, object1_class, object2_class,robot_class)
